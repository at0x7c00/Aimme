 ...
 */
@Repository
public class MonthSummaryDaoImpl extends BaseDaoImpl<MonthSummary> implements IMonthSummaryDao {
    @Override
    public Long findListRowCount(MonthSummary monthSummary) {
        Criteria criteria = getSession().createCriteria(MonthSummary.class).setProjection(Projections.rowCount());
	...
	public Long findHistoryListRowCount(MonthSummary monthSummary,Page pageInfo) {
		...
		AuditQuery query = queryCreator2.forRevisionsOfEntity(MonthSummary.class, false, true);
		query.add(AuditEntity.property("manageKey").eq(monthSummary.getManageKey()));
    ...
    @Override
    public List<MonthSummary> findListPage(MonthSummary monthSummary, Page pageInfo){
    	Criteria criteria = getSession().createCriteria(MonthSummary.class).setFirstResult(pageInfo.getStartIndex()).setMaxResults(pageInfo.getNumPerPage());
        queryCause(criteria,monthSummary);
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<HistoryRecord<MonthSummary>> findHistoryListPage(MonthSummary monthSummary, Page pageInfo) {
		AuditReader reader = AuditReaderFactory.get(getSession());
		AuditQueryCreator queryCreator2 = reader.createQuery();
		AuditQuery query = queryCreator2.forRevisionsOfEntity(MonthSummary.class, false, true);
...
criteria.add(Restrictions.ge("createTime",monthSummary.getCreateTimeStart()));
}
if(monthSummary.getCreateTimeEnd()!=null){
criteria.add(Restrictions.le("createTime",monthSummary.getCreateTimeEnd()));
}
       if(monthSummary.getIncome()!=null
){
		criteria.add(Restrictions.eq("income",monthSummary.getIncome()));
}
       if(monthSummary.getBalance()!=null
){
		criteria.add(Restrictions.eq("balance",monthSummary.getBalance()));
}
       if(monthSummary.getOutcome()!=null
){
		criteria.add(Restrictions.eq("outcome",monthSummary.getOutcome()));
}
    }
	@Override
	public MonthSummary findByVersion(Integer version) {
		AuditReader reader = AuditReaderFactory.get(getSession());
		AuditQueryCreator queryCreator2 = reader.createQuery();
		AuditQuery query = queryCreator2.forEntitiesAtRevision(MonthSummary.class, version);
	...
	public List<MonthSummary> findByKey(Page pageInfo,String queryKey) {
		Criteria criteria = getSession().createCriteria(MonthSummary.class).setFirstResult(pageInfo.getStartIndex()).setMaxResults(pageInfo.getNumPerPage()).add(Restrictions.like("manageKey", queryKey,MatchMode.ANYWHERE));
		return criteria.list();
	}
	@Override
