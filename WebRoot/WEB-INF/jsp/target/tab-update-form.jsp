	...
	<td>
		...
		<input type="hidden" name="rowGuard${trIndex}" value="1"/>
				...
				<td>
								...
								<input name="${propName}[${trIndex}].progress"
									id="${propName}[${trIndex}].progress" type="text"
									   value="<c:out value="${tmpTmpBean.progress}"/>"
				...
				<td>
								...
								value="<fmt:formatDate pattern='${applicationScope.EN_YEAR_MONTH_DAY_HOUR_MIN_SEC}' value='${tmpTmpBean.deadLine}'/>" class="date_not_required textInput valid"/>
								</label>
				</td>
				<td>
								<label class="input">
										<input name="${propName}[${trIndex}].resource"
											...
											class="textInput "
											 value="<c:out value="${tmpTmpBean.resource}"/>"
											maxlength="255" />
								</label>
				</td>
				<td>
								<label class="input">
										<input name="${propName}[${trIndex}].checkItem"
											...
											class="textInput "
											 value="<c:out value="${tmpTmpBean.checkItem}"/>"
											maxlength="255" />
								</label>
				</td>
				<td>
								<label class="input">
										<input name="${propName}[${trIndex}].output"
											...
											class="textInput "
											 value="<c:out value="${tmpTmpBean.output}"/>"
											maxlength="255" />
								</label>
				</td>
				<td>
										...
										<select id="${propName}[${trIndex}].parent.id" name="${propName}[${trIndex}].parent.id" class=" comboxed">
											...
											<c:forEach items="${targetList}" var="target" >
												<option value="${target.id}" ${tmpTmpBean.parent.id eq target.id ? 'selected' : ''}><c:out value="${target.name}"/></option>
											</c:forEach>
										</select><i></i>
										</label>
				</td>
				...
				<td>
								...
								<option value=""><spring:message code="base.common.selectone"/></option>
									<c:forEach items="${emergencyMap}" var="entry" >
										<option value="${entry.key}" ${tmpTmpBean.emergency eq entry.key ? 'selected' : ''}><c:out value="${entry.value}"/></option>
									</c:forEach>
								</select><i></i>
								</label>
				</td>